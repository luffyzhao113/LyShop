    ...
    <i-drawer title="添加类目" :loading="loading">
        <Form ref="formCreate" :model="create" :label-width="80" :rules="ruleValidate">
            ...
            <Row class="search_filters-box" :gutter="10">
                ...
                <Col span="24">
                    <Row v-for="(item, index) in create.search_filters" :key="index" :gutter="10">
                        <Col span="4" align="center">
                        ...
                        <Col span="2" align="center">
                            <FormItem :label-width="0">
                                <Button @click="remove(create.search_filters, index)" size="small">删除</Button>
                            </FormItem>
                        </Col>
                    </Row>
                </Col>
                <Col offset="18" span="6">
                    <FormItem :label-width="0">
                        <Button long size="small" @click="addAreaItem(create.search_filters)" type="dashed">添加一行
                        </Button>
                    </FormItem>
                </Col>
            </Row>
        ...
        methods: {
            submit(name) {
                this.validate(name).then(() => {
                    this.loading = true;
                    this.$http.post(`goods/category`, this.create).then(() => {
            ...
            handleChangeOk() {
                if (this.filter.value) {
                    if (!oneOf(this.filter.value, this.create.search_filters[this.filter.index].values)) {
                        this.create.search_filters[this.filter.index].values.push(
                            this.filter.value
                        );
